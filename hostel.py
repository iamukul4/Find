...
def login_owner_validation():
	...
	if flag==1:
		...
		if flag==3:
			flash('your hostel already uploaded , please update or show')
			return render_template("owner_login.html")
		else:
...
def owner_login_with_update_success():
	if (request.method=='POST'):
		...
		if os.path.exists(path):
			...
			con=pymysql.connect(host='localhost',user='root',password='',database='hostel')
			cur=con.cursor()
			cur.execute('update owner_info set Image1=%s , Image2=%s , Image3=%s , Image4=%s , Hostel_type=%s , Hostel_name=%s , Owner_name=%s , Total_rooms=%s, Available_rooms=%s, Contact_no=%s, Alternate_no=%s, Room_charge=%s, Address=%s, Landmark=%s, City=%s, Pincode=%s, State=%s, Country=%s, Cooler_air_conditioner_refrigerator=%s where Email=%s ',(img1,img2,img3,img4,hostel_type,hostel_name,owner_name,total_rooms,available_rooms,contact_no,alternate_no,fees,address,landmark,city,pincode,state,country,car,email))
			con.commit()
			con.close()
...
def search_hostel(hostelname):
	...
	if (request.method=='POST'):
		...
		con=pymysql.connect(host='localhost',user='root',password='',database='hostel')
		cur=con.cursor()
		cur.execute("select * from owner_info where Hostel_name=%s",hostelname)
		row=cur.fetchone()
